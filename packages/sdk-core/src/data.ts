...
export class DataNamespace implements DataClientInternals {
  ...
  async query<T>(dataType: string, q: FromQueryOptions): Promise<{ records: RecordEnvelope<T>[]; nextCursor?: string }> {
    ...
    return request(this.opts.url, `/api/v1/data/${encodeURIComponent(this.opts.appId)}/${encodeURIComponent(dataType)}`, {
      method: "GET",
  ...
  async create<T>(dataType: string, data: Partial<T>): Promise<RecordEnvelope<T>> {
    await this.auth.ensureFreshToken();
    return request<RecordEnvelope<T>>(this.opts.url, `/api/v1/data/${encodeURIComponent(this.opts.appId)}/${encodeURIComponent(dataType)}`, {
      method: "POST",
      body: { data },
      headers: this.auth.authHeader()
    });
  ...
  async update<T>(recordId: string, patch: Partial<T>): Promise<RecordEnvelope<T>> {
    await this.auth.ensureFreshToken();
    return request<RecordEnvelope<T>>(this.opts.url, `/api/v1/data/record/${encodeURIComponent(recordId)}`, {
      method: "PATCH",
      body: { data: patch },
      headers: this.auth.authHeader()
    });
  ...
  async ops<T>(recordId: string, ops: FieldOp[]): Promise<RecordEnvelope<T>> {
    await this.auth.ensureFreshToken();
    return request<RecordEnvelope<T>>(this.opts.url, `/api/v1/data/record/${encodeURIComponent(recordId)}/ops`, {
      method: "POST",
      body: { ops },
      headers: this.auth.authHeader()
    });
