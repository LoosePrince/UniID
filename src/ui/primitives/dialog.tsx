    ...
    <DialogPortal>
      ...
      >
        ...
        <DialogPrimitive.Close
          className="absolute right-3 top-3 rounded-sm p-1 text-ink-400 hover:text-ink-900 hover:bg-cream-100 transition-colors"
          aria-label="Close"
        >
          <X className="h-4 w-4" />
        </DialogPrimitive.Close>
...
export function DialogHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn("flex flex-col gap-1 p-5 border-b border-ink-100", className)} {...props} />;
}
...
export function DialogFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "flex flex-col-reverse gap-2 p-5 border-t border-ink-100 sm:flex-row sm:justify-end",
        className
      )}
      {...props}
    />
...
>(function DialogTitle({ className, ...props }, ref) {
  return (
    <DialogPrimitive.Title
      ref={ref}
      className={cn("text-md font-semibold text-ink-900 leading-snug", className)}
      {...props}
    />
  );
});
