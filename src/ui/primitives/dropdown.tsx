...
function menuItemClassName(tone: MenuTone, inset?: boolean) {
  return cn(
    "relative flex min-h-9 cursor-default select-none items-center gap-2.5 rounded-xl px-3 py-2 text-sm font-medium text-ink-700",
    "outline-none transition-[background,color,box-shadow,transform] duration-150",
    "focus:bg-accent-50 focus:text-ink-900 focus:shadow-[inset_0_0_0_1px_rgba(119,111,218,0.16)]",
    ...
    "data-[state=open]:bg-accent-100 data-[state=open]:text-ink-900 data-[state=open]:shadow-[inset_0_0_0_1px_rgba(119,111,218,0.2)]",
    "data-[disabled]:pointer-events-none data-[disabled]:opacity-45",
    "[&>svg]:size-4 [&>svg]:shrink-0",
    tone === "accent" && "text-accent-700 focus:bg-accent-50 focus:text-accent-800 data-[highlighted]:bg-accent-50 data-[highlighted]:text-accent-800",
    tone === "danger" && "text-danger-600 focus:bg-danger-50 focus:text-danger-700 data-[highlighted]:bg-danger-50 data-[highlighted]:text-danger-700",
    inset && "pl-8"
  );
}
  ...
  return (
    <DropdownMenuPortal>
      <DropdownPrimitive.SubContent
        ...
        className={cn(
          "surface-elevated z-50 min-w-40 overflow-hidden rounded-2xl border border-white/70 p-1.5 shadow-lg",
  ...
  return (
    <DropdownMenuPortal>
      <DropdownPrimitive.Content
        ...
        sideOffset={sideOffset}
        className={cn(
          "surface-elevated z-50 min-w-44 overflow-hidden rounded-2xl border border-white/70 p-1.5 shadow-lg",
          "data-[state=open]:animate-scale-in",
          className
        )}
        {...props}
...
>(function DropdownMenuLabel({ className, ...props }, ref) {
  return (
    <DropdownPrimitive.Label
      ref={ref}
      className={cn("px-3 pb-1.5 pt-2 text-2xs font-medium uppercase tracking-[0.14em] text-ink-400", className)}
      {...props}
    />
  );
});
...
>(function DropdownMenuSeparator({ className, ...props }, ref) {
  return <DropdownPrimitive.Separator ref={ref} className={cn("my-1.5 h-px bg-ink-100/90", className)} {...props} />;
});
